...
public class DefaultDAOImpl implements DAO {
    ...
    public <T extends DataSet> void save(T object){
        object.setId(getSequenceNextValue(object.getClass()));
    ...
    public <T extends DataSet> T load(long id, Class<T> clz){
        Executor executor = new Executor(connection);
        try {
            return executor.execSelectById(createSelectQuery(clz),id, result ->{
                if(result.next()){
    ...
    public String getQuotedTableName(Class<?> clz){
    ...
    public String createSelectQuery(Class<?> clz){
    ...
    public String createInsertQuery(Class<?> clz){
    ...
    private Long getSequenceNextValue(Class<?> clz){
        Executor executor = new Executor(connection);
        String query = "SELECT "+getTableName(clz)+"_SEQ.nextval FROM DUAL";
        try {
            return executor.execSimpleSelect(query, result ->{
                if(result.next()){
